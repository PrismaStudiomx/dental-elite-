...
export default function App() {
  ...
  // Estado para el cuadro de texto de la urgencia
  ...
  // ENVIAR WHATSAPP DE URGENCIAS directas
        ...
        {horariosDelDia.map(h => {
          ...
          return (
            <button 
              ...
              onClick={() => { setReserva({...reserva, horario: h, fecha: formatearFecha(fechaSeleccionada)}); setStep(3); }}
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans antialiased overflow-x-hidden">
      ...
      {/* NAVBAR */}
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-200/60 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => document.getElementById('modulo-urgencias').scrollIntoView({behavior: 'smooth'})} 
            className="text-red-600 border border-red-200 bg-red-50/50 px-4 py-2.5 rounded-full text-xs font-bold hover:bg-red-600 hover:text-white transition-all"
          >
            SOS URGENCIAS
          </button>
          <button 
            onClick={() => document.getElementById('reservar').scrollIntoView({behavior: 'smooth'})} 
            className="bg-slate-900 text-white px-5 py-2.5 rounded-full text-xs font-bold hover:bg-blue-600 transition-all shadow-sm"
          >
            AGENDAR CITA
          </button>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="flex gap-4">
              <button 
                onClick={() => document.getElementById('reservar').scrollIntoView({behavior: 'smooth'})}
                className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-bold hover:bg-slate-900 transition-all shadow-lg shadow-blue-200"
              >
                Agendar Cita Normal
              </button>
            </div>
          ...
          <div className="relative">
            <img src="https://images.unsplash.com/photo-1629909607103-68d80509f61b?q=80&w=1000" className="rounded-[2.5rem] shadow-2xl border border-white" alt="Especialista Dental" />
          </div>
        </div>
      </section>

      {/* TRATAMIENTOS */}
      <section className="py-24 bg-slate-100/60 px-6 border-y border-slate-200/40">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h3 className="text-3xl font-black uppercase tracking-tight text-slate-900">Tratamientos Avanzados</h3>
            <p className="text-slate-400 text-sm mt-2">Tecnología clínica aplicada a tu bienestar.</p>
          </div>
              ...
              <div key={idx} className="bg-white rounded-[2rem] overflow-hidden border border-slate-200/60 shadow-sm flex flex-col justify-between">
                ...
                <div className="p-8 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <h4 className="font-bold text-xl text-slate-900 mb-2">{s.nombre}</h4>
      ...
      {/* NUEVA SECCIÓN: MÓDULO EXCLUSIVO DE URGENCIAS */}
      <section id="modulo-urgencias" className="pt-24 px-6 max-w-4xl mx-auto">
        <div className="bg-gradient-to-br from-red-600 to-rose-700 rounded-[2.5rem] p-8 md:p-12 text-white shadow-xl shadow-red-100 relative overflow-hidden">
          <div className="absolute right-[-20px] top-[-20px] text-red-500/20 pointer-events-none">
            <AlertCircle size={240} strokeWidth={1} />
          </div>
          
          <div className="relative z-10 max-w-2xl">
            <div className="flex items-center gap-2 bg-red-700/50 border border-red-500/30 w-max px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider mb-6">
            ...
            <p className="text-red-100 text-sm md:text-base mb-8 leading-relaxed">
              Dolor severo, traumatismos, inflamación severa o pérdida de una pieza. Escribe qué te ocurre aquí abajo; saltarás la fila de espera y un odontólogo de guardia te atenderá de inmediato por WhatsApp.
            </p>
            ...
            <div className="space-y-3">
              <textarea 
                ...
                placeholder="Ej: Tengo un dolor insoportable en una muela desde hace 2 horas / Se me rompió un diente frontal por un golpe..."
                className="w-full p-4 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 outline-none text-sm h-24 placeholder:text-red-200/80 text-white focus:bg-white/15 focus:border-white/50 transition-all resize-none"
              />
              <button 
                onClick={enviarUrgenciaWhatsApp}
                className="w-full sm:w-auto bg-white text-red-700 px-8 py-4 rounded-2xl font-black text-sm hover:bg-slate-900 hover:text-white transition-all shadow-lg shadow-red-800/40 flex items-center justify-center gap-2.5"
              >
      ...
      {/* PANEL DE RESERVAS CITA NORMAL */}
      <section id="reservar" className="py-24 px-6 max-w-4xl mx-auto">
        <div className="bg-white rounded-[2.5rem] border border-slate-200/80 shadow-xl overflow-hidden">
          <div className="grid md:grid-cols-3">
            <div className="bg-slate-900 p-10 text-white flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-800">
              <div>
                <span className="text-blue-500 text-[10px] uppercase font-black tracking-widest block mb-2">Citas Online</span>
                <h3 className="text-2xl font-black leading-tight">PANEL DE<br/>RESERVA</h3>
              </div>
              <div className="space-y-6 mt-10 md:mt-0">
                {[1,2,3].map(n => (
                  <div key={n} className={`flex items-center gap-3 transition-opacity duration-300 ${step === n ? 'opacity-100' : 'opacity-40'}`}>
                    <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-[10px] font-bold ${step === n ? 'bg-blue-600 border-blue-600 text-white' : 'border-white text-white'}`}>{n}</div>
                    <span className="text-[10px] uppercase font-black tracking-widest">{n === 1 ? 'Tratamiento' : n === 2 ? 'Horario' : 'Confirmar'}</span>
                  </div>
                ))}
              </div>
            </div>

            <div className="md:col-span-2 p-10 bg-slate-50/50">
                  ...
                  <motion.div key="s1" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-3">
                    ...
                    {servicios.map(s => (
                      <button 
                        ...
                        className="w-full p-5 bg-white rounded-2xl border border-slate-200/80 text-left flex justify-between items-center hover:border-blue-600 hover:bg-blue-50/20 transition-all group"
                      >
                        <div>
                          <p className="font-bold text-slate-800 text-base">{s.nombre}</p>
                          <p className="text-xs text-slate-400 mt-0.5">{s.precio}</p>
                        </div>
                ...
                {step === 3 && (
                  <motion.div key="s3" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-6">
                    <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100/70">
                      ...
                      <h5 className="font-bold text-slate-900 text-lg">{reserva.servicio}</h5>
                    ...
                    <div>
                      ...
                      <textarea 
                        ...
                        className="w-full p-4 bg-white rounded-2xl border border-slate-200 outline-none text-sm h-28 focus:border-blue-600 transition-colors resize-none"
                        onChange={(e) => setReserva({...reserva, nota: e.target.value})}
                      />
                    </div>
                    <div className="flex gap-3">
                      <button onClick={() => setStep(1)} className="px-5 bg-slate-200 text-slate-700 rounded-2xl text-xs font-bold hover:bg-slate-300 transition-colors">Volver</button>
                      <button 
                        onClick={registrarCitaEnBase}
                        disabled={cargando}
                        className="flex-1 bg-blue-600 text-white py-4 rounded-2xl font-black text-sm shadow-md shadow-blue-200 hover:bg-blue-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                      >
      ...
      {/* FAQ */}
      <section className="py-24 px-6 max-w-3xl mx-auto border-t border-slate-200/60">
        <h3 className="text-2xl font-black text-center mb-12 text-slate-900">PREGUNTAS <span className="text-blue-600">FRECUENTES</span></h3>
        <div className="space-y-3">
          {faqs.map((f, i) => (
            <div key={i} className="border border-slate-200/60 rounded-2xl overflow-hidden bg-white shadow-sm">
              <button 
                onClick={() => setOpenFaq(openFaq === i ? null : i)}
                className="w-full p-5 flex justify-between items-center hover:bg-slate-50 transition-all text-left"
              >
              ...
              <AnimatePresence>
                {openFaq === i && (
                  <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                    <p className="p-5 pt-0 text-slate-400 text-xs leading-relaxed border-t border-slate-100 bg-slate-50/40">{f.a}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
      ...
      <footer className="bg-slate-900 text-white py-16 px-6">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-10 text-sm text-slate-400">
          <div>
